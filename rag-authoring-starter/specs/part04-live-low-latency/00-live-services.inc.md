...
:: A live service for which the client polls and reloads the MPD following
    <code><b>MPD</b>@minimumUpdatePeriod</code> to discover newly available Segments and Periods.
...
- The first Period <span class=modal-keyword>shall</span> start at or after the zero point of the MPD timeline
  (with a <code><b>Period</b>@start</code> value of 0 seconds or greater).
- The last Period <span class=modal-keyword>may</span> have a <code><b>Period</b>@duration</code>, in which case it has a fixed
  duration. If without <code><b>Period</b>@duration</code>, the last Period in a dynamic
...
- If <code><b>Period</b>@start</code> is present, PSwc[i] is the sum of <code><b>MPD</b>@availabilityStartTime</code>
    (AST) and `@start`.
...
- <code><b>MPD</b>@type</code> <span class=modal-keyword>shall</span> be set to `dynamic`.
- <code><b>MPD</b>@availabilityStartTime</code> <span class=modal-keyword>shall</span> be present and provides the wall-clock anchor
    for the Media Presentation timeline.
- <code><b>MPD</b>@publishTime</code> <span class=modal-keyword>shall</span> be present and <span class=modal-keyword>shall</span> be updated whenever the MPD content
    changes, so that clients can detect a changed MPD.
- Where the presentation end time is not known in advance,
    <code><b>MPD</b>@minimumUpdatePeriod</code> <span class=modal-keyword>shall</span> be present; the Period end time of the last
    Period is then obtained as the sum of `NOW` and <code><b>MPD</b>@minimumUpdatePeriod</code>.
...
the absence of <code><b>MPD</b>@timeShiftBufferDepth</code>, the start of the time shift buffer is
...
<code><b>MPD</b>@suggestedPresentationDelay</code> attribute to provide a suggested presentation
delay. Clients <span class=modal-keyword>should</span> use <code><b>MPD</b>@suggestedPresentationDelay</code> when provided by the MPD.
...
- <code><b>MPD</b>@minimumUpdatePeriod</code> <span class=modal-keyword>shall</span> be set to a value consistent with the rate at
...
- On each change, the service <span class=modal-keyword>shall</span> write a new MPD with an updated
    <code><b>MPD</b>@publishTime</code>; clients detect the change by comparing publish times.
- <code><b>MPD</b>@minimumUpdatePeriod</code> <span class=modal-keyword>may</span> be set to `0` to indicate that the client <span class=modal-keyword>should</span>
...
time span of duration <code><b>MPD</b>@minimumUpdatePeriod</code> starting from the moment the MPD
...
The presence or absence of <code><b>MPD</b>@minimumUpdatePeriod</code> <span class=modal-keyword>shall</span> be used by DASH services
to signal whether and when the MPD might be updated:

- A nonzero value for <code><b>MPD</b>@minimumUpdatePeriod</code> defines the MPD validity duration
...
- The value 0 for <code><b>MPD</b>@minimumUpdatePeriod</code> indicates that the MPD has no
...
- Absence of the <code><b>MPD</b>@minimumUpdatePeriod</code> attribute indicates an infinite
...
- Remove the <code><b>MPD</b>@minimumUpdatePeriod</code> attribute.
- Cease performing MPD updates.

This signals to clients that no more content will be added to the MPD.

Upon detecting the removal of <code><b>MPD</b>@minimumUpdatePeriod</code>, clients <span class=modal-keyword>should</span> present a
...
on-demand viewing, the service <span class=modal-keyword>may</span> change <code><b>MPD</b>@type</code> to `static` when
<code><b>MPD</b>@minimumUpdatePeriod</code> is removed or do so at a later time. Clients <span class=modal-keyword>shall</span> not
...
<code><b>MPD</b>@minimumUpdatePeriod</code> interval. They can do so as often or as rarely as they
...
- Determine the live edge from the MPD timeline and the synchronized wall-clock
    time, taking <code><b>MPD</b>@suggestedPresentationDelay</code> into account where present to
    select the initial presentation time.
- Begin playout at a presentation time that leaves sufficient buffer within the
    time-shift buffer (<code><b>MPD</b>@timeShiftBufferDepth</code>) to avoid rebuffering, while
